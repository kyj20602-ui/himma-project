...
const STORAGE_KEY = "himma-building-checker-v9";
...
function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("himma-building-checker-v8") || localStorage.getItem("himma-building-checker-v7") || localStorage.getItem("himma-building-checker-v6") || localStorage.getItem("himma-building-checker-v5");
...
function performReview({ scroll = true } = {}) {
  ...
  // 건폐율은 기본 설정의 엘리베이터 면적을 건축면적에서 자동 제외합니다.
  // 층별 1F 입력값과 무관하게 항상 기본 엘리베이터 면적을 기준으로 하므로 누락되지 않습니다.
  const bcrElevatorArea = Math.min(buildingArea, n(state.settings.defaultElevatorArea));
  const bcrArea = Math.max(0, buildingArea - bcrElevatorArea);
  const permittedBuildingArea = siteArea * maxBcr / 100;
  const bcr = bcrArea / siteArea * 100;
  ...
  lastResult = {
    floors, buildingArea, bcrElevatorArea, bcrArea, permittedBuildingArea, bcr,
    remainingBuildingArea: permittedBuildingArea - bcrArea,
    farArea, permittedFarArea, far, remainingFarArea: permittedFarArea - farArea,
...
function renderSummary(r) {
  ...
  const cards = [
    ["허용 건축면적", `${fmt(r.permittedBuildingArea)}㎡`, `대지면적 × ${fmt(state.settings.maxBcr, 0)}%`, status(true)],
    ["계획 건폐율", `${fmt(r.bcr)}%`, `입력 ${fmt(r.buildingArea)}㎡ − 1F EL ${fmt(r.bcrElevatorArea)}㎡ = ${fmt(r.bcrArea)}㎡`, status(r.bcr <= state.settings.maxBcr)],
    ["건폐율 여유면적", `${fmt(r.remainingBuildingArea)}㎡`, r.remainingBuildingArea >= 0 ? "허용 건축면적까지 남은 면적" : "허용 건축면적 초과", status(r.remainingBuildingArea >= 0)],
    ["허용 용적률 면적", `${fmt(r.permittedFarArea)}㎡`, `대지면적 × ${fmt(state.settings.maxFar, 0)}%`, status(true)],
    ["계획 용적률", `${fmt(r.far)}%`, `산입면적 ${fmt(r.farArea)}㎡`, status(r.far <= state.settings.maxFar)],
    ["용적률 여유면적", `${fmt(r.remainingFarArea)}㎡`, r.remainingFarArea >= 0 ? "허용 용적률 면적까지 남은 면적" : "허용 용적률 면적 초과", status(r.remainingFarArea >= 0)],
...
function renderPrintReport(r) {
  ...
  // PDF 상단은 핵심 지표만 표시합니다. 허용면적은 별도 카드로 반복하지 않습니다.
  const metrics = [
    ...
    ["주차 여유면적", `${fmt(r.parkingRemainingArea)}㎡`]
  ];
  document.getElementById("printMetrics").innerHTML = metrics
    .map(([k, v]) => `<div><span>${k}</span><strong>${v}</strong></div>`)
    .join("");

  document.getElementById("printFloorRows").innerHTML = r.floors
    .map(f => `<div class="print-floor-row"><strong>${f.name}</strong><span>${fmt(f.grossArea)}㎡</span><span>${fmt(f.elevatorExcluded)}㎡</span><span>${fmt(f.netArea)}㎡</span></div>`)
    .join("");

  // 산정·허용·여유를 항목별 한 줄에 묶어 PDF 하단 넘침을 방지합니다.
  document.getElementById("printCalculation").innerHTML = `
    <div class="print-calc-row">
      <span class="print-calc-title">건폐율</span>
      <span><em>산정</em><strong>${fmt(r.bcrArea)}㎡ · ${fmt(r.bcr)}%</strong></span>
      <span><em>허용</em><strong>${fmt(state.settings.maxBcr, 0)}%</strong></span>
      <span><em>여유</em><strong>${fmt(r.remainingBuildingArea)}㎡</strong></span>
    </div>
    <div class="print-calc-row">
      <span class="print-calc-title">용적률</span>
      <span><em>산정</em><strong>${fmt(r.farArea)}㎡ · ${fmt(r.far)}%</strong></span>
      <span><em>허용</em><strong>${fmt(state.settings.maxFar, 0)}%</strong></span>
      <span><em>여유</em><strong>${fmt(r.remainingFarArea)}㎡</strong></span>
    </div>
    <div class="print-calc-row">
      <span class="print-calc-title">주차</span>
      <span><em>산정</em><strong>${fmt(r.parkingRaw, 2)}대 (${r.requiredParking}대)</strong></span>
      <span><em>허용</em><strong>${fmt(state.settings.targetParking, 0)}대</strong></span>
      <span><em>여유</em><strong>${fmt(r.parkingRemainingArea)}㎡</strong></span>
    </div>`;

  const printImage = document.getElementById("printImage");
  const placeholder = document.getElementById("printImagePlaceholder");
  if (state.imageDataUrl) {
    printImage.src = state.imageDataUrl;
...
function resetProject() {
  ...
  localStorage.removeItem("himma-building-checker-v8");
  localStorage.removeItem("himma-building-checker-v7");
  localStorage.removeItem("himma-building-checker-v6");
  localStorage.removeItem("himma-building-checker-v5");
