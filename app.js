  ...
  floors.forEach(floor => {
    ...
    card.innerHTML = `
      ...
      <div class="mini-results">
        <span>엘리베이터 제외 <b>${format(excludingElevator)}㎡</b> / ${format(excludingElevator / PYEONG)}평</span>
        <span>순사용면적 <b>${format(netArea)}㎡</b> / ${format(netArea / PYEONG)}평</span>
        <span>주차지수 <b>${format(parkingRaw, 3)}대</b></span>
      </div>`;

    const fields = card.querySelector('.floor-fields');
    fields.append(
      field('전체 바닥면적', floor.grossArea, '㎡', v => { floor.grossArea = v; renderFloors(); calculate(); }),
      field('피난동선 면적', floor.evacuationArea, '㎡', v => { floor.evacuationArea = v; renderFloors(); calculate(); })
    );
    ...
    const parkingField = field('주차 산정면적', parkingArea, '㎡', v => { floor.parkingAreaManual = v; calculate(); });
    ...
    card.querySelector('.parking-check').addEventListener('change', e => { floor.includeInParking = e.target.checked; calculate(); });
    card.querySelector('.elevator-check').addEventListener('change', e => { floor.hasElevator = e.target.checked; renderFloors(); calculate(); });
    card.querySelector('.elevator-area').addEventListener('input', e => { floor.elevatorArea = num(e.target.value); renderFloors(); calculate(); });
