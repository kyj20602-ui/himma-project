...
const STORAGE_KEY = "himma-building-checker-v8";
...
function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("himma-building-checker-v7") || localStorage.getItem("himma-building-checker-v6") || localStorage.getItem("himma-building-checker-v5");
...
function renderPrintReport(r) {
  ...
  const metrics = [
    ...
    ["계획 건폐율", `${fmt(r.bcr)}%`],
    ["건폐율 산정면적", `${fmt(r.bcrArea)}㎡`],
    ["허용 건축면적", `${fmt(r.permittedBuildingArea)}㎡`],
    ["건폐율 여유면적", `${fmt(r.remainingBuildingArea)}㎡`],
    ["계획 용적률", `${fmt(r.far)}%`],
    ["용적률 산입면적", `${fmt(r.farArea)}㎡`],
    ["허용 용적률 면적", `${fmt(r.permittedFarArea)}㎡`],
    ["용적률 여유면적", `${fmt(r.remainingFarArea)}㎡`],
    ["주차대수 / 여유", `${fmt(r.parkingRaw, 2)}대 · ${fmt(r.parkingRemainingArea)}㎡`]
  ];
  ...
  document.getElementById("printCalculation").innerHTML = `
    <div><span>건폐율 산정</span><strong>${fmt(r.buildingArea)}㎡ − 기본 EL ${fmt(r.bcrElevatorArea)}㎡ = ${fmt(r.bcrArea)}㎡ · ${fmt(r.bcr)}%</strong></div>
    <div><span>건폐율 허용·여유</span><strong>허용 ${fmt(r.permittedBuildingArea)}㎡ · 여유 ${fmt(r.remainingBuildingArea)}㎡</strong></div>
    <div><span>용적률 산정</span><strong>${fmt(r.farArea)}㎡ · ${fmt(r.far)}%</strong></div>
    <div><span>용적률 허용·여유</span><strong>허용 ${fmt(r.permittedFarArea)}㎡ · 여유 ${fmt(r.remainingFarArea)}㎡</strong></div>
