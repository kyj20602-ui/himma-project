...
const STORAGE_KEY = "himma-building-checker-v7";
...
function loadState() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem("himma-building-checker-v6") || localStorage.getItem("himma-building-checker-v5");
...
function performReview({ scroll = true } = {}) {
  ...
  const firstFloor = floors.find(f => f.name === "1F");
  const bcrElevatorArea = firstFloor?.hasElevator ? firstFloor.elevatorArea : 0;
...
function renderPrintReport(r) {
  ...
  const metrics = [
    ...
    ["건폐율 여유", `${fmt(r.remainingBuildingArea)}㎡`],
    ["용적률 산입면적", `${fmt(r.farArea)}㎡`],
    ["허용 용적률 면적", `${fmt(r.permittedFarArea)}㎡`],
    ["용적률 여유", `${fmt(r.remainingFarArea)}㎡`],
  ...
  document.getElementById("printCalculation").innerHTML = `
    <div><span>건폐율 산정</span><strong>${fmt(r.buildingArea)} − ${fmt(r.bcrElevatorArea)} = ${fmt(r.bcrArea)}㎡ / ${fmt(r.bcr)}%</strong></div>
    <div><span>건폐율 허용·여유</span><strong>${fmt(r.permittedBuildingArea)}㎡ / ${fmt(r.remainingBuildingArea)}㎡</strong></div>
    <div><span>용적률 허용·여유</span><strong>${fmt(r.permittedFarArea)}㎡ / ${fmt(r.remainingFarArea)}㎡</strong></div>
