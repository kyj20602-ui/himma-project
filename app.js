...
function renderPrintReport(r) {
  ...
  document.getElementById("printFloorRows").innerHTML = r.floors.map(f => `<div class="print-floor-row"><strong>${f.name}</strong><span>전체 ${fmt(f.grossArea)}㎡</span><span>EL 제외 ${fmt(f.elevatorExcluded)}㎡</span><span>실사용 ${fmt(f.netArea)}㎡</span></div>`).join("");
...
function handleImage(file) {
  ...
  reader.onload = () => { state.imageDataUrl = reader.result; updateImagePreview(); localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); };
  reader.readAsDataURL(file);
}

function saveProject() { readInputs(); localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); alert("현재 입력값을 저장했습니다."); }
function resetProject() {
  if (!confirm("모든 입력값과 첨부 이미지를 초기화할까요?")) return;
  state = structuredClone(defaultState); lastResult = null; localStorage.removeItem(STORAGE_KEY);
...
function initialize() {
  buildFloorCards(); syncInputs();
  document.getElementById("reviewButton").addEventListener("click", () => performReview());
  document.getElementById("pdfButton").addEventListener("click", printPdf);
  ...
  document.getElementById("removeImageButton").addEventListener("click", () => { state.imageDataUrl = ""; document.getElementById("projectImage").value = ""; updateImagePreview(); });
  document.getElementById("defaultElevatorArea").addEventListener("change", (e) => {
    const value = n(e.target.value); state.floors.forEach(f => { if (f.hasElevator) f.elevatorArea = value; }); buildFloorCards();
  });
}
initialize();
